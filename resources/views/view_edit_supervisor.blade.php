...
<html lang="en">
  <head>
    ...
    <title>CPM - Edit Supervisor</title>
...
<div class="container">
  <div class="mr-auto main">
   ...
   <form class="mainform" action="{{url('update_supervisor/'.$row->id)}}" method="POST" enctype="multipart/form-data">
  @csrf

  <div class="form-group ">
    <label for="name" class="inplabel">Name</label> 
      <input id="name" name="name" type="text" value="{{$row->supervisor_name}}" placeholder="Name" class="form-control {{ ($errors->any() && $errors->first('name')) ? 'is-invalid' : '' }}" >
  ...
  <div class="form-group ">
    <label for="Email" class="inplabel">Email</label> 
      <input id="email" name="supervisor_email" type="Email" value="{{$row->supervisor_email}}" placeholder="Email" class="form-control {{ ($errors->any() && $errors->first('supervisor_email')) ? 'is-invalid' : '' }}" >
    <p class="invalid-feedback" style="color: red; display: none;" id="em"> {{$errors->first('supervisor_email')}} </p>
        @if($errors->any())
    <p class="invalid-feedback" style="color: red;"> {{$errors->first('supervisor_email')}} </p>
  @endif
  </div>
  <div class="form-group ">
    <label for="ID" class="inplabel">ID</label> 
      <input id="ID" name="supervisor_id" type="number" value="{{$row->supervisor_id}}" placeholder="ID" class="form-control {{ ($errors->any() && $errors->first('supervisor_id')) ? 'is-invalid' : '' }}" >
        @if($errors->any())
    <p class="invalid-feedback" style="color: red;"> {{$errors->first('supervisor_id')}} </p>
  @endif
  </div> 
  <div class="form-group ">
    <div class="">
